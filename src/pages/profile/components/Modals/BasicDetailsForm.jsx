...
const BasicDetailsForm = (props) => {
    ...
    return (
        <div className="z-10 fixed backdrop-blur w-screen h-full left-0 top-0 bg-[#00000080] flex items-center justify-center">
            <form
                className="relative w-10/12 md:w-3/4 lg:w-3/5 grid p-4 md:p-8 bg-white rounded-lg h-4/5 md:h-5/6 lg:h-3/4 overflow-scroll overflow-x-hidden"
                onSubmit={updateProfile}
            >
                <h3 className="sticky md:relative w-fit text-xl md:text-2xl lg:text-4xl text-custom-green mb-8">
                    Edit Basic Details
                </h3>
                <div className="grid md:grid-cols-2 gap-1 md:gap-3">
                    ...
                    />
                    <label
                        className="relative grid gap-2 w-full mb-6"
                        onClick={handleChangeDropdown}
                    >
                        <span className="text-base md:text-[20px]/[22px] font-medium text-green-700 dark:text-green-500">
                            State
                        </span>
                        {stateDropdown ? (
                            <Select
                                closeModal={handleChangeDropdown}
                                value="state"
                            />
                        ) : (
                            <button
                                type="button"
                                className="w-full h-full py-2 px-5  border-2 border-custom-grey bg-slate-50 rounded-lg"
                            >
                                {props.formData.StateofAddress
                                    ? props.formData.StateofAddress
                                    : "Select a state"}
                            </button>
                        )}
                    ...
                    <label
                        className="relative grid gap-2 w-full mb-6"
                        onClick={handleLgaChangeDropdown}
                    >
                        <span className="text-base md:text-[20px]/[22px] font-medium text-green-700 dark:text-green-500">
                            L.G.A
                        </span>
                        ...
                        stateDropdown == false ? (
                            <Select
                                closeModal={handleLgaChangeDropdown}
                                value={props.formData.StateofAddress}
                            />
                        ) : (
                            <button
                                type="button"
                ...
                <div className=" bg-white sticky bottom-[-24px] flex items-center gap-5 justify-end py-4">
                    <button
                        ...
                        className="px-3 py-3 md:px-8 md:py-4 rounded-lg text-xs md:text-base bg-rose-600 text-white"
                    >
                        Cancel
                    </button>
                    <button
                        type="submit"
                        className="px-3 py-3 md:px-8 md:py-4 rounded-lg text-xs md:text-base text-white bg-[#22C55E] w-fit"
                    >
                        Saves Changes
                    </button>
                </div>
            ...
            <CustomModal isOpen={isOpen}>
                <ModalContent closeModal={closeModal} heading="Your profile has been successfully updated!" primaryText="Close the modal to continue using the protal." secondaryText="Thank you!" />
            </CustomModal>
        </div>
    );
};
